...
"""
RIR data generation script with discrete coordinate sampling and original metadata standard
"""
...
class SimulationConfig:
    ...
    n_sources: int = 1
    group_size: int = 5  # Number of rooms per coordinate combination
    mic_type: str = "ambeovr"
    seed: int = utils.SEED
    
    # Azimuth and elevation ranges and steps
    azimuth_range: Tuple[float, float] = (-180.0, 180.0)
    elevation_range: Tuple[float, float] = (-79.0, 79.0)
    angular_step: float = 5.0  # Step size in degrees for both azimuth and elevation
    
    # Distance range and step
    distance_min: float = 0.8
    distance_max: float = 3.0
    distance_step: float = 0.5  # Step size in meters

    max_retries: int = 5  # Maximum retries for RIR simulation
...
def generate_discrete_coordinates(config: SimulationConfig) -> List[Tuple[float, float, float]]:
    """
    Generate all discrete coordinate combinations based on the configuration
    
    Args:
        config: Simulation configuration containing coordinate parameters
        
    Returns:
        List of all (azimuth, elevation, distance) combinations in degrees and meters
    """
    # Generate azimuth values
    azimuth_start, azimuth_end = config.azimuth_range
    azimuth_values = np.arange(azimuth_start, azimuth_end + config.angular_step, config.angular_step)
    
    # Generate elevation values
    elevation_start, elevation_end = config.elevation_range
    elevation_values = np.arange(elevation_start, elevation_end + config.angular_step, config.angular_step)
    
    # Generate distance values
    distance_values = np.arange(config.distance_min, config.distance_max + config.distance_step, config.distance_step)
    
    logger.info(f"Generated coordinate grid:")
    logger.info(f"  Azimuth: {len(azimuth_values)} values from {azimuth_start}° to {azimuth_end}° (step: {config.angular_step}°)")
    logger.info(f"  Elevation: {len(elevation_values)} values from {elevation_start}° to {elevation_end}° (step: {config.angular_step}°)")
    logger.info(f"  Distance: {len(distance_values)} values from {config.distance_min}m to {config.distance_max}m (step: {config.distance_step}m)")
    ...
    for azimuth in azimuth_values:
        for elevation in elevation_values:
            for distance in distance_values:
                coordinates.append((float(azimuth), float(elevation), float(distance)))
    
    logger.info(f"  Total coordinate combinations: {len(coordinates)}")
    logger.info(f"  Total planned simulations: {len(coordinates) * config.group_size}")
...
def create_world_state_and_simulate(mesh_path: str, config: SimulationConfig, coordinates: tuple) -> Dict[str, Any]:
    ...
    logger.debug(f"Processing {space_name}")
    ...
    try:
        ...
        for attempt in range(config.max_retries):
            try:
                ...
                logger.debug(f"Successfully added microphone and emitter for {space_name}")
                break  # Exit loop if successful
            except Exception as e:
                logger.warning(f"Attempt {attempt + 1}/{config.max_retries} failed for {space_name}: {str(e)}")
                if attempt == config.max_retries - 1:
                    raise RuntimeError(f"Failed to add microphone/emitter after {config.max_retries} attempts")
...
def initialize_metadata_file(config: SimulationConfig, output_dir: str, total_groups: int) -> str:
    """
    ...
    Args:
        ...
        total_groups: Total number of coordinate groups
    ...
    # Create initial metadata structure (same as original)
    initial_metadata = {
        "seed": config.seed,
        "dataset": "Gibson Habitat",
        "version": "1.0",
        "sampling_method": "discrete_grid",
        "config": {
            ...
            "coordinate_sampling": {
                ...
                "angular_step": config.angular_step,
                "distance_min": config.distance_min,
                "distance_max": config.distance_max,
                "distance_step": config.distance_step
            }
        },
        "groups": {},
        "summary": {
...
def save_simulation_data(sim_data: Dict[str, Any], output_dir: str, 
    ...
    Args:
        ...
        metadata_path: Path to the metadata file
    """
    # Create filename (same format as original)
    ...
    # Create metadata entry (same format as original)
    metadata_entry = {
        "fname": fname,
...
def generate_rir_dataset(config: SimulationConfig, dataset_dir: str, output_dir: str) -> None:
    """
    Main function to generate RIR dataset with discrete coordinate sampling
    ...
    # Generate all discrete coordinates
    all_coordinates = generate_discrete_coordinates(config)
    total_groups = len(all_coordinates)
    
    # Initialize metadata file
    metadata_path = initialize_metadata_file(config, output_dir, total_groups)
    
    # Get Gibson mesh files
    gibson_files = get_gibson_meshes(dataset_dir)
    
    logger.info(f"Starting dataset generation with {total_groups} coordinate combinations (groups)")
    logger.info(f"Processing {config.group_size} rooms per coordinate combination")
    logger.info(f"Total planned simulations: {total_groups * config.group_size}")
    
    # Track statistics
    total_processed = 0
    total_failed = 0
    
    # Process each coordinate combination as a group
    for group_idx, coordinates in enumerate(all_coordinates):
        azimuth, elevation, distance = coordinates
        logger.info(f"Processing group {group_idx + 1}/{total_groups}: "
                   f"az={azimuth:.1f}°, el={elevation:.1f}°, dist={distance:.1f}m")
        ...
        # Randomly select rooms for this group (same as original behavior)
        if len(gibson_files) < config.group_size:
            logger.warning(f"Not enough rooms available. Using all {len(gibson_files)} rooms instead of {config.group_size}")
            selected_rooms = gibson_files.copy()
        else:
        ...
        for room_idx, mesh_path in enumerate(selected_rooms):
            try:
                ...
                if sim_data is not None:
                    ...
                    logger.debug(f"Successfully processed {sim_data['space_name']} "
        ...
        if (group_idx + 1) % 50 == 0:
            logger.info(f"Progress: {group_idx + 1}/{total_groups} groups completed. "
...
def main():
    ...
    config = SimulationConfig(
        n_sources=1,
        group_size=12,  # Number of rooms per coordinate combination
        mic_type="ambeovr",
        ...
        angular_step=10.0,  # 10 degrees step for both azimuth and elevation
        
        distance_min=0.8,
        distance_max=2.0,
        distance_step=1.0,  # 0.5 meter step for distance
    )
    
    # Paths
    project_root = utils.get_project_root()
    dataset_dir = os.path.join(project_root, "/scratch/ssd1/matterport3d/gibson_habitat/gibson")
    output_dir = os.path.join(project_root, "/scratch/ssd1/DINOSAUD_Dataset_Discrete")
